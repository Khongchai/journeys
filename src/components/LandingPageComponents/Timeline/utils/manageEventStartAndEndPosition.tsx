...
export default function manageEventStartAndEndPosition(eventData: Array<any>) {
  let allEvents = document.getElementsByClassName(
    "event"
  ...
  for (let i = 0; i < allEventsLength; i += topicPerNode) {
    ...
    //manage row
    allEvents[i].style.gridRow = checkIfColumnIsAvailableAtRowX(
      begin1,
      end1,
      rowCheck
    )
    allEvents[i + 1].style.gridRow = checkIfColumnIsAvailableAtRowX(
      begin2,
      end2,
      rowCheck
    )
    allEvents[i + 2].style.gridRow = checkIfColumnIsAvailableAtRowX(
      begin3,
      end3,
      rowCheck
    )
