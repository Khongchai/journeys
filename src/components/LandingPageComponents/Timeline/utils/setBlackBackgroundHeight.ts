export default function setBlackBackgroundHeight() {
  ...
  if (eventsWrapper) {
    ...
    let blackBackground: HTMLElement = document.getElementById(
      "black-background"
    )
    ...
    if (windowWidth < currentBreakpoints) {
      ...
      blackBackground.style.height = `${
        parseInt(eventsComputedStyle.getPropertyValue("height")) +
        navbarHeight +
        extraHeight
      }px`
    } else {
      blackBackground.style.height = `${parseInt(
        eventsComputedStyle.getPropertyValue("height")
      )}px`
    }
  }
}
