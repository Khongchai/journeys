...
export function checkIfOffScreen()
{
    ...
    var homeButton = document.getElementById("home-button");
    let buttonWidth: number = parseInt(window.getComputedStyle(homeButton).getPropertyValue("width"));
    let arbitraryCompensation = 11;

    checkOffset(buttonWidth, thresholdRight, thresholdBottom, arbitraryCompensation);

    homeButton.style.transform = `translate3d(${offsetX}px, ${offsetY}px, 0)`;
}

function checkOffset(buttonWidth: number, thresholdRight: number, thresholdBottom: number, arbitraryCompensation: number)
{
...
function buttonOffScreenRight(buttonWidth: number, thresholdRight: number): boolean
{
...
function buttonOffScreenBottom(buttonWidth: number, thresholdBottom: number): boolean
{
