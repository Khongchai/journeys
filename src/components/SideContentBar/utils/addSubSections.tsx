...
export default function manageSubSections(
  sectionName: string,
  operation: string
) {
  let sidebarSection: HTMLElement = document.getElementById(
    `sidebar${sectionName}`
  )
  let paragraphSection: HTMLElement = document.getElementById(
    `${sectionName}paragraph`
  )
  let subSections: HTMLCollectionOf<Element> = paragraphSection.getElementsByTagName(
    "strong"
  )
