...
export default function manageSubSectionsHighlight() {
  let activeSubSections = document.getElementsByClassName(
    "subsection-anchor"
  ...
  for (let i = 0; i < activeSubSectionsLength; i++) {
    ...
    let targetParagraph: HTMLElement = document.getElementById(
      `${sectionName}paragraph`
    )

    //if there exist a sub section of id ${sectionName}
    if (targetParagraph) {
      let paragraphHeight: number = parseInt(
        window.getComputedStyle(targetParagraph).getPropertyValue("height")
      )

      if (
        elementIsVisibleInViewport(
          targetParagraph.getBoundingClientRect().y,
          bottomThreshold,
          topThreshold,
          paragraphHeight + targetParagraph.getBoundingClientRect().y
        )
      ) {
